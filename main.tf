...
output "adresses" {
  description = "Adresse IPv4 de chaque machine du laboratoire"
  value       = { for nom, vm in multipass_instance.lab : nom => vm.ipv4 }
}

output "hosts" {
  description = "Lignes à placer dans le /etc/hosts du bastion"
  value = join("\n", [
    for nom, vm in multipass_instance.lab :
    "${vm.ipv4} ${nom} #lab" if nom != "bastion"
  ])
}
